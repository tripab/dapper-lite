...
static const char *test_thread_local_set_get()
{
    ...
    mu_assert("set_current(NULL) should clear current span",
              current == NULL);
...
static const char *test_thread_local_isolation()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        pthread_create(&threads[i], NULL, thread_local_isolation_worker, &thread_ids[i]);
    }
...
static const char *test_context_inject_extract()
{
    ...
    mu_assert_eq("inject should return correct size", TRACE_CONTEXT_WIRE_SIZE, len);
...
static const char *test_context_endianness()
{
    ...
    mu_assert_eq("round-trip should preserve trace_id",
                 0x0123456789ABCDEFULL, ctx.trace_id);
...
static const char *test_span_create_from_context()
{
    ...
    mu_assert_eq("span should have remote parent_span_id",
                 ctx.span_id, span->parent_span_id);
    mu_assert_eq("trace should be updated with extracted trace_id",
...
static const char *test_cross_process_trace_continuity()
{
    ...
    /* Verify continuity */
    mu_assert_eq("backend trace_id should match frontend",
                 original_trace_id, backend_span->trace_id);
    mu_assert_eq("backend parent should be frontend span",
                 frontend_span_id, backend_span->parent_span_id);
