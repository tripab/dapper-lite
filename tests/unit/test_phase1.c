...
static const char *test_span_create_finish()
{
    ...
    mu_assert_eq("root span should have parent_span_id = 0", 0, span->parent_span_id);
    mu_assert_str_eq("span should have correct name", "test_span", span->name);
    mu_assert("span should have non-zero start time", span->monotonic_start_ns != 0);
    mu_assert_eq("unfinished span should have end time = 0", 0, span->monotonic_end_ns);

    span_finish(span);

    mu_assert("finished span should have non-zero end time", span->monotonic_end_ns != 0);
...
static const char *test_span_name_truncation()
{
    ...
    mu_assert("span name should be truncated", strlen(span->name) < strlen(long_name));
...
static const char *test_span_duration()
{
    ...
    mu_assert_eq("unfinished span should have 0 duration", 0, span_duration_ns(span));
...
static const char *test_parent_child_hierarchy()
{
    ...
    mu_assert_eq("parent should have parent_span_id = 0", 0, parent->parent_span_id);

    span_t *child1 = span_create(trace, parent, "child1");
    mu_assert("child1 should have parent", child1->parent == parent);
    mu_assert_eq("child1 should have parent's span_id",
                 parent->span_id, child1->parent_span_id);
...
static const char *test_annotation_storage()
{
    ...
    mu_assert_str_eq("first annotation key should match",
                     "key1", span->annotations[0].key);
    mu_assert_str_eq("first annotation value should match",
                     "value1", span->annotations[0].value);
...
static const char *test_annotation_overflow()
{
    ...
    mu_assert_eq("should have max annotations",
                 MAX_ANNOTATIONS, span->annotation_count);

    /* Try to add one more (should be silently ignored) */
    span_annotate(span, "overflow", "ignored");
    mu_assert_eq("should still have max annotations",
                 MAX_ANNOTATIONS, span->annotation_count);
...
static const char *test_annotation_null_safety()
{
    ...
    mu_assert_eq("should have 0 annotations after NULL calls",
                 0, span->annotation_count);
